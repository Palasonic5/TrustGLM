...
class DataArguments:
    ...
    train_file: Optional[str] = field(
        default="Baselines/LLaGA/dataset/sup/cora_sup/sampled_2_10_train_nettack_sbert.jsonl"
    )
    ...
    def __init__(self,
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if "." in dataset:
                ...
                repeat=int(ds[1])
                dataset=ds[0]
            if "arxiv_sup" in dataset:
                data_path = "dataset/ogbn-arxiv_sup/processed_data.pt"
            elif "products_sup"  in dataset:
                data_path = "dataset/ogbn-products_sup/processed_data.pt"
            elif "pubmed_sup"  in dataset:
                data_path = "dataset/pubmed_sup/processed_data.pt"
            elif "cora_sup"  in dataset:
                data_path = "dataset/cora_sup/processed_data.pt"
            elif "computers_sup" in dataset:
                data_path = "dataset/amazon-computers_sup/processed_data.pt"
            elif "sports_sup" in dataset:
                data_path = "dataset/amazon-sports_sup/processed_data.pt"
            else:
            ...
            if data_args.template == "ND":
                ...
                self.structure_emb = torch.load(
                    f"Baselines/LLaGA/dataset/laplacian_{data_args.use_hop}_{data_args.sample_neighbor_size}.pt")
