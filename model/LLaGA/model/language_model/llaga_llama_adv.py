...
class LlagaLlamaForCausalLM(LlamaForCausalLM, LlagaMetaForCausalLM):
    ...
    def forward(
        ...
        print(f"input ids after first preparation {input_ids}")
        outputs = self.model(
        ...
        # Generate adversarial embeddings and compute adversarial loss
        if graph_emb is not None and labels_new is not None:
            # Disable autocast during adversarial computation
            with torch.cuda.amp.autocast(enabled=False):
                ...
                loss_original.backward(retain_graph=True)  # Backward pass for gradient
                epsilon = torch.tensor(epsilon, dtype=graph_emb.dtype, device=graph_emb.device)
                perturbation = epsilon * graph_emb.grad.sign()
                graph_emb_adv = graph_emb + perturbation
            # Enable autocast again for remaining computations
            with torch.cuda.amp.autocast(enabled=True):
                print(f"Input IDs before prepare_inputs_labels_for_multimodal: {input_ids}")
                # Recompute embeddings and logits with adversarial embeddings
