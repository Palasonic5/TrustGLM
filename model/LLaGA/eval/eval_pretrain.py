...
def load_pretrain_embedding_graph(data_dir, pretrained_embedding_type):
    ...
    elif pretrained_embedding_type == "BoW":
        pretrained_emb = torch.load(os.path.join(data_dir, f"{pretrained_embedding_type}_x.emb")).to(torch.float)
    elif pretrained_embedding_type == "GIA":
        pretrained_emb = torch.from_numpy(smat_util.load_matrix(os.path.join(data_dir, f"{pretrained_embedding_type}_x.emb")).astype(np.float32))
...
def eval_model(args):
    ...
    if args.task in  ["nc", "nd", "nda", "nctext", "nc-mlabel"]:
        if args.template == "HO":
            prompt_file = args.prompt_file
        else:
            prompt_file = args.prompt_file
        data_path = f"dataset/{args.dataset}/processed_data.pt"
    ...
    if args.template == "ND":
        ...
        structure_emb = torch.load(
            f"Baselines/LLaGA/dataset/laplacian_{args.use_hop}_{args.sample_neighbor_size}.pt")
        ...
        if args.dataset in SMALL_DATASETS and args.task == "lp":
            ...
            for q in questions:
                ...
                if "lp" in  args.task:
                    ...
                    mask[idx[0]] = True
                    mask[idx[1]] = True
                elif args.task  in ["nc", "nd", "nctext","nc_mlabel"]:
