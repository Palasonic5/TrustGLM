...
def eval_arxiv_nc(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/ogbn-arxiv_semi/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_arxiv_nc_sub(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/LLaGA/dataset/sup/ogbn-arxiv_sup/balanced_subgraph_no_isolates.pt")
    data_ori=torch.load("/scratch/xs2334/TrustGLM/Baselines/LLaGA/dataset/sup/ogbn-arxiv_sup/processed_data.pt")
    labels=data_ori.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_amazon_photo_nc(res_path):
    data=torch.load("dataset/amazon-photo_semi/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
    ...
    with open(res_path, 'r') as f:
        for line in f:
            ...
            ans = res["text"]
            y=ys[res["question_id"]]
            short_label = short_labels[int(y)]
            label=labels[int(y)]
            if label.lower().strip() == ans.lower().strip():
                strict_correct+=1
                overall_correct+=1
...
def eval_amazon_history_nc(res_path):
    data=torch.load("dataset/amazon-history_sup/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_amazon_children_nc(res_path):
    data=torch.load("dataset/amazon-children_sup/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_amazon_sports_nc(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/amazon-sports_semi/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_amazon_computers_nc(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/amazon-computers_semi/processed_data.pt")
    labels=data.label_texts
    short_labels = [l[0:5] for l in labels]
    ys=data.y.numpy().tolist()
...
def eval_lp(res_path):
    all_sample=0
    correct = 0
    file_path = '/gpfsnyu/scratch/jd5849/LLaGA/dataset/cora_semi/edge_sampled_2_10_only_test.jsonl'
    gt_data_list = []
...
def eval_products_nc(res_path):
    eval_set = set()
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/ogbn-products_semi/processed_data.pt")
    labels=data.label_texts
    ys=data.y.numpy().tolist()
...
def eval_pubmed_nc(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/pubmed_semi/processed_data.pt")
    labels=data.label_texts
...
def eval_cora_nc(res_path):
    data=torch.load("/scratch/xs2334/TrustGLM/Baselines/new/dataset/cora_semi/processed_data.pt")
    labels=data.label_texts
...
if __name__ == '__main__':
    ...
    func_dict = {
        ...
        "computers_sup": {
            "nc": eval_amazon_computers_nc
        },
        "computers_semi": {
            "nc": eval_amazon_computers_nc
        },
        "photo_sup": {
            "nc": eval_amazon_photo_nc
        },
        "photo_semi": {
            "nc": eval_amazon_photo_nc
        },
        "sports_sup": {
            "nc": eval_amazon_sports_nc
        },
        "sports_semi": {
            "nc": eval_amazon_sports_nc
        },
        "children_sup": {
            "nc": eval_amazon_children_nc
        },
        "history_sup": {
            "nc": eval_amazon_history_nc
        },
