...
from graphprompter.src.model.gnn import load_gnn_model
...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('/scratch/qz2086/hard-label-attack/graphprompter/Llama-2-7b-hf', use_fast=False, revision=kwargs["revision"])
        self.tokenizer.pad_token_id = 0
        self.tokenizer.padding_side = 'left'

        model = AutoModelForCausalLM.from_pretrained(
            '/scratch/qz2086/hard-label-attack/graphprompter/Llama-2-7b-hf',
    ...
    def inference(self, samples):
        ...
        # print("samples['question']:", samples["question"])
        # print("Type of samples['question']:", type(samples["question"]))
        questions = self.tokenizer(samples["question"], add_special_tokens=False)
        desriptions = self.tokenizer(samples["desc"], add_special_tokens=False)

        # encode special tokens
        # pad_embeds = self.word_embedding(torch.tensor(self.tokenizer.pad_token_id).to(device)).unsqueeze(0)
        pad_id_tensor = torch.tensor(self.tokenizer.pad_token_id, device=device)
        pad_embeds = self.word_embedding(pad_id_tensor).unsqueeze(0)
